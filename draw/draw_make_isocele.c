...
/*   Updated: 2016/01/12 14:04:24 by                  ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_area	ft_draw_make_isocele(t_line line, int mediane)
{
	...
	ao = sqrtf((line.dx / 2) * (line.dx / 2) + (line.dx / 2) * (line.dy / 2));
	if (ao > (float)mediane)
	...
	triangle.c.x = intersection.x - (int)((float)(line.dx / 2) / coef);
	triangle.c.y = intersection.y - (int)((float)(line.dy / 2) / coef);
	return (triangle);
}
